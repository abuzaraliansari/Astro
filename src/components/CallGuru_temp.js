    ...
    try {
      ...
      if (response.data.success) {
        ...
        try {
          ...
          const formattedDate = new Date(selectedDateObj.sqlDate).toLocaleDateString('en-IN', {
            weekday: 'long',
            year: 'numeric',
            month: 'long',
            day: 'numeric'
          ...
          const emailData = {
            ...
            customerPhone: user.mobile_number || 'Not provided',
            guruName: guruInfo.name,
            consultationType: consultationType.name,
            bookingDate: formattedDate,
            startTime: selectedSlot.label,
            duration: consultationType.duration_minutes,
...
console.log('📧 Booking email data:', emailData);
          ...
          await sendEmailNotification('BOOKING_BUSINESS_ALERT', null, emailData);
          console.log('✅ Business notification email sent');

        } catch (emailError) {
          console.error('⚠️ Email notification failed (booking still successful):', emailError);
  ...
  const handleCancelBooking = async (bookingId) => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        const fullBookingData = existingBookings.find(b => b.id === bookingId);

        try {
          console.log('📧 Sending cancellation emails...');
          console.log('📊 Booking data:', cancelledBooking);
          console.log('📊 Full booking data:', fullBookingData);

          // Format date for email
          const bookingDate = fullBookingData?.booking_date || cancelledBooking?.bookingDate;
          ...
          // ✅ FIXED: Use correct field names from your booking data
          const cancelEmailData = {
            customerName: user.full_name || user.name,
            customerEmail: user.email,
            guruName: fullBookingData?.guru_name || cancelledBooking?.guruName || 'Guru',
            consultationType: fullBookingData?.consultation_name || cancelledBooking?.consultationName || 'Consultation',
            bookingDate: formattedDate,
            startTime: fullBookingData ? formatTime(fullBookingData.start_hour, fullBookingData.start_minute) : 'Time',
            creditsRefunded: refundedCredits,
            bookingId: bookingId,
            cancellationReason: 'Cancelled by user from web app'
          };

          console.log('📧 Email data to send:', cancelEmailData);

          // Send customer cancellation confirmation
          await sendEmailNotification('CANCELLATION_CONFIRMATION', user.email, cancelEmailData);
          console.log('✅ Customer cancellation email sent');

          // Send business cancellation notification
          await sendEmailNotification('CANCELLATION_BUSINESS_ALERT', null, cancelEmailData);
          console.log('✅ Business cancellation notification sent');

        } catch (emailError) {
          console.error('⚠️ Email notification failed (cancellation still successful):', emailError);
